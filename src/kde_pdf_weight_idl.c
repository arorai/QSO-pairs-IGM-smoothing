...
/******************************************************************************/
IDL_LONG kde_pdf_weight_idl(int argc,void *argv[])
{
  ...
  wi         =     (float *)argv[5]; // weights for data points
  pdf        =     (float *)argv[6]; // pdf evaluated at xpdf
  ...
  for(ipdf=0; ipdf<npdf; ipdf++) {
    ...
    for (ii=0; ii<ni; ii++) { 
      ...
      if(abs(xarg) < 10.0) temp += wi[ii]*exp(-xarg*xarg/2.0);
    }
    pdf[ipdf]=temp*norm;
  }
