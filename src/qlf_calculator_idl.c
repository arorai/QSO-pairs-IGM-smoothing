...
IDL_LONG qlf_calculator_idl(int argc,void *argv[])
{

	double nu,REDSHIFT;
	IDL_LONG FIT_KEY;
	IDL_LONG n_l_bol_pts1;
	double log_l_bol_min,log_l_bol_max,d_log_l_bol;
	double *l_bol_grid,*l_band_grid,*M_AB_grid,*S_nu_grid,*phi_bol_grid;
	int n_l_bol_pts;

	//int FIT_KEY=0;
	int VOCAL=0;	// set to 0 to only print the output luminosity/QLF lists (i.e. no explanatory notes)
	...
	nu            =    *(double *)argv[0];  // frequency
	REDSHIFT      =    *(double *)argv[1];  // redshift
	FIT_KEY       =    *(IDL_LONG *)argv[2];  // FIT_KEY
	n_l_bol_pts1  =    *(IDL_LONG *)argv[3];  // number of points
	log_l_bol_min =   *(double *)argv[4];  // max l_bol
	log_l_bol_max =   *(double *)argv[5];  // max l_bol
	d_log_l_bol   =   *(double *)argv[6];  // max l_bol
	l_bol_grid    =    (double *)argv[7]; // l_bol_grid
	l_band_grid   =    (double *)argv[8];
	M_AB_grid     =    (double *)argv[9];
	S_nu_grid     =    (double *)argv[10];
	phi_bol_grid  =    (double *)argv[11];

	n_l_bol_pts   = (int)n_l_bol_pts1;
